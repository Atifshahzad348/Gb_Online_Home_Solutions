...
//                             <button 
//                                 type="submit" 
//                                 className="nav-btn btn w-100 fw-bold"
//                                 disabled={loading}
//                             >
...
const ProfessionalLogIn = () => {
    const navigate = useNavigate();
    const { storeTokenInLS, setUser } = useAuth(); // Now setUser is available
    const [loginData, setLoginData] = useState({
        cnic: "",
        password: ""
    });
    ...
    const submitCall = async (data) => {
        ...
        try {
            ...
            if(response.ok){
                ...
                // Now setUser is available, so we can use it properly
                setUser({
                    userId: res_data.userId,
                    userType: 'professional',
                    cnic: loginData.cnic,
                    name: res_data.user?.name || 'Professional',
                    profession: res_data.user?.profession || ''
                });
